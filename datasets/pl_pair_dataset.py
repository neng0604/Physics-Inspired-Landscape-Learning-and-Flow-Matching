...
class PocketLigandPairDataset_ba(Dataset):
    ...
    def __init__(self, raw_path, transform=None, version='final'):
        ...
        self.affinity_path = os.path.join('/data3/zhoujingyuan/kgdiff', 'affinity_info_complete.pkl')
        self.transform = transform
        self.db = None
        self.keys = None
...
class PocketLigandPairDataset_volume(Dataset):
    ...
    def __init__(self, raw_path, transform=None, version='final'):
        ...
        self.index_path = os.path.join(self.raw_path, 'index.pkl')
        self.processed_path = os.path.join(os.path.dirname(self.raw_path),
                                           os.path.basename(self.raw_path) + f'_processed_{version}.lmdb') # 共有166500条数据
        self.raw_affinity_path = os.path.join('/data/qianhao', TYPES_FILENAME)
        self.affinity_path = os.path.join('/data3/zhoujingyuan/kgdiff', 'affinity_info_complete.pkl')
        self.volume_path = os.path.join('/data2/zhoujingyuan/moflow', 'volume_info.pkl')
        self.transform = transform
        self.db = None
        self.keys = None
        self.affinity_info = None
    ...
    def _update_affinity(self, sid, affinity):
        if self.db is None:
            self._connect_db()
        txn = self.db.begin(write=True)
        data = pickle.loads(txn.get(sid))
    ...
    def _update_volume(self, sid, volume):
        if self.db is None:
            self._connect_db()
        txn = self.db.begin(write=True)
        data = pickle.loads(txn.get(sid))
        ...
        elif data['volume'] == None:
            data['volume'] = volume['volume'],
            data['area'] = volume['area'],
    ...
    def _inject_affinity(self, sid, ligand_path):
        if ligand_path[:-4] in self.affinity_info:
            affinity = self.affinity_info[ligand_path[:-4]]
            self._update_affinity(sid, affinity)
        else:
            raise AttributeError(f'affinity_info has no {ligand_path[:-4]}')

    def _inject_volume(self, sid, ligand_path):
        if ligand_path[:-4] in self.volume_info:
            volume = self.volume_info[ligand_path[:-4]]
            self._update_volume(sid, volume)
        else:
            raise AttributeError(f'volume_info has no {ligand_path[:-4]}')
    ...
    def get_ori_data(self, idx):
        ...
        if 'affinity' not in data:
            ...
            self._inject_affinity(key, data['ligand_filename'])
            data = pickle.loads(self.db.begin().get(key))
        if 'volume' not in data or data['volume']==None:
            self._load_volume_info()
            self._inject_volume(key, data['ligand_filename'])
            data = pickle.loads(self.db.begin().get(key))
