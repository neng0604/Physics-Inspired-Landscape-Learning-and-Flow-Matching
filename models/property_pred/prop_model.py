...
from models.common import compose_context_prop, ShiftedSoftplus, compose_context_chem


def get_encoder(config):
    if config.name == 'egnn' or config.name == 'egnn_enc' or config.name == 'egnn_qed':
        net = EnEquiEncoder(
...
class QedPredNet(nn.Module):
    def __init__(self, config, ligand_atom_feature_dim, output_dim=1):
        super(QedPredNet, self).__init__()
        self.config = config
        self.hidden_dim = config.hidden_channels
        self.output_dim = output_dim
        self.ligand_atom_emb = nn.Linear(ligand_atom_feature_dim, self.hidden_dim)
    ...
    def forward(self, ligand_pos, ligand_atom_feature, batch_ligand):
        ...
        h_ctx, pos_ctx, batch_ctx = compose_context_chem(
            h_ligand=h_ligand,
            pos_ligand=ligand_pos,
            batch_ligand=batch_ligand,
        )
