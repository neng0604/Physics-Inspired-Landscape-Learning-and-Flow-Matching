...
def compose_context_chem( h_ligand, pos_ligand, batch_ligand):
    batch_ctx = torch.cat([batch_ligand], dim=0)
    sort_idx = batch_ctx.argsort()

    batch_ctx = batch_ctx[sort_idx]
    h_ctx = torch.cat([h_ligand], dim=0)[sort_idx]
    pos_ctx = torch.cat([pos_ligand], dim=0)[sort_idx]
