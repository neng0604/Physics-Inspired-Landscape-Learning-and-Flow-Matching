...
class NormalizeVina(object):
    ...
    def __init__(self, mode='pl_ba'):
        super().__init__()
        self.mode = mode
        if mode == 'pl_ba' or self.mode == 'pl_ba_abs':
            self.max_v = 0
            self.min_v = -16   # -17.20
        elif mode == 'pdbbind':
            self.max_v = 16
            self.min_v = 0
        elif self.mode == 'pl_ba_gaussian':
            self.mean_v =  -8.19
            self.std_v = 4.02
        else:
            raise ValueError
    ...
    def _trans(self, vina_score):
        if self.mode == 'pl_ba':
            ...
            return (vina_score - self.mean_v) / self.std_v
        elif self.mode == 'pl_ba_debug':
            return vina_score
        elif self.mode == 'pl_ba_abs':
            return vina_score / self.min_v
        else:
            raise ValueError
