...
from models.property_pred.prop_model import PropPredNet, PropPredNetEnc, QedPredNet
...
def get_dataloader(train_set, val_set, test_set, config, is_need_protein):
    if is_need_protein:
...
def get_model(config, protein_atom_feat_dim, ligand_atom_feat_dim):
    ...
    elif config.model.encoder.name == 'egnn_qed':
        model = QedPredNet(
            config.model,
